<template>
  ...
  <div class="indicator-title q-my-xl full-width">
    <h4>Ranking de entidades contratantes con mayor cantidad de registros incompletos por procedimiento de contratación</h4>
  </div>
  
  <div class="form-container">
    <RankingEntidadesContratantesIncompletosPorProcedimientoInputs 
...
<script setup>
import { reactive, watchEffect, toRaw, defineProps } from 'vue';
import RankingEntidadesContratantesIncompletosPorProcedimientoInputs from './RankingEntidadesContratantesIncompletosPorProcedimientoInputs.vue';
import DataVisualization from '@/components/data-visualization/DataVisualization.vue';
...
const indicatorDataModel = reactive(
  {
    paramsInformacionIncompletaValue: paramsInformacionIncompleta[1],
    contractingProceduresSelectedValue: contractingProcedures[19],
...
const dataFilter = function(){
  ...
  const result = dataIndicator[indicatorDataModel.contractingProceduresSelectedValue]
  ...
  for (let provincia in result2) {
    for (let year in result2[provincia]) {
      for (let month in result2[provincia][year]) {
        for (let entidad in result2[provincia][year][month]) {
          if (indicatorDataModel.paramsInformacionIncompletaValue === 'monto'){
            result3[provincia][year][month][entidad] = result2[provincia][year][month][entidad][0]
          } else if (indicatorDataModel.paramsInformacionIncompletaValue === 'fecha') {
            result3[provincia][year][month][entidad] = result2[provincia][year][month][entidad][1]
          }
        }
      }
    }
